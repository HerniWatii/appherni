...
class Auth extends CI_Controller 
{
	...
	public function registration()
	{
		...
		$this->db->sendEmail($token, 'verify');
		$this->session->set_flashdata('message', '</div class="alert alert-success" role="alert">
		Congratulations!your account has been created. Please Activate your Account</div>');
		redirect('auth');
		}
	}
		//function untuk mengirim cerivikasi ke mail
	private function _sendEmail($token, $type)
		{
			$config = array();
			$config['protocol'] = 'smtp';
			$config['smtp_host'] = 'ssl://smtp.googlemail.com';
			$config['smtp_user'] = 'hernimisnari01@gmail.com'; //email yang dirubah keamanannya
			$config['smtp_pass'] = 'mabfxogqzcbfpixn'; //isikan password dr kelola akun gmail
			$config['smtp_port'] = 465;
			$config['mailtype'] = 'html';
			$config['charset'] = 'utf-8';
			//$config = array(
			//'protocol' => 'smpt',
			//'smpt_host' => 'smpt.mailtrap.io',
			//'smpt_port' => 2525,
			//'smpt_user' => '51739a034cb411',
			//'smpt_pass' => 'd0d6a0f025db3a',
			//'crlf' => "\r\n",
			//'newline' => "\r\n"
			//);
			...
			$this->email->from('hernimisnari01@gmail.com', 'Web Programing SMKN8Jember');
	...
	{
		{
			...
			if ($user) {
				...
				$this->session->set_flashdata('message', '<div claa="alert alert-danger"
	...
	public function changePassword() 
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Password has been change!Please login</div>');
			redirect('auth');
		}
	}
